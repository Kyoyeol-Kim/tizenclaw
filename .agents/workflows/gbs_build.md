...
1. **빌드를 실행합니다**: Tizen `gbs build`는 git repository의 commit된 소스를 기준으로 tarball을 만들지만, `--include-all` 옵션을 주면 커밋하지 않은 사항들도 포함하여 빌드합니다.
   - **처음 빌드** (또는 spec 파일의 의존성이 변경된 경우): `gbs build -A x86_64 --include-all`
   - **반복 수정 빌드** (의존성 변경 없이 코드만 수정한 경우): `gbs build -A x86_64 --include-all --noinit`

   `--noinit` 옵션은 빌드 환경 초기화를 건너뛰어 빌드 속도를 크게 향상시킵니다. spec 파일에 새로운 의존성(BuildRequires 등)을 추가하지 않은 경우에 사용하면 유용합니다.

   > [!NOTE]
   > 빌드 과정에서 의존성 관련 오류가 발생하면 `--noinit` 옵션을 제거하고 다시 빌드하세요. `--noinit`은 이전 빌드 환경을 재사용하므로, 새로 추가된 의존성이 설치되지 않아 빌드가 실패할 수 있습니다.
